...
def init_db():
    with app.app_context():
        ...
        ''')
        # Initialize analytics record if not exists
        if not db.execute('SELECT 1 FROM app_analytics WHERE id = 1').fetchone():
...
def compatibility_test():
    if request.method == 'POST':
        ...
        if not (resume_file and allowed_file(resume_file.filename)):
            flash('Invalid resume file format', 'error')
            return redirect(request.url)
            
        if not (job_desc_file and allowed_file(job_desc_file.filename)):
        ...
        try:
            ...
            if not os.path.exists(resume_path) or not os.path.exists(job_desc_path):
                flash('Failed to save files', 'error')
                return redirect(request.url)
